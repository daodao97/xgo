...
func RegisterAPI[Req any, Resp any](handler func(*gin.Context, Req) (*Resp, error)) gin.HandlerFunc {
	...
	// EnableOpenAPI 开关与 go run 判定都是 xgo 默认生成流程的策略（go run 模式
	// 源码在盘上，GenerateOpenAPIDoc 动态生成；编译产物读取已生成的
	// openapi.json），放在本层而非登记核心，外部自定义流程自行把关。
	if Args.EnableOpenAPI && utils.IsGoRun() {
		RegisterAPIDoc[Req, Resp](handler, wrappedHandler)
	}

	return wrappedHandler
}

// RegisterAPIDoc 是 OpenAPI 文档登记核心：从 sourceHandler 定义上方的源码注释
// 提取元数据（@summary / @description / @tags / @deprecated / @accept /
// @produce，普通函数与 method value 都支持），成功则把 wrappedHandler 与
// Req / Resp 类型写入文档注册表；解析不出来时跳过登记并返回 false。
//
// sourceHandler 是承载注释的业务函数；wrappedHandler 是最终注册到 gin 路由上的
// 函数值（文档生成按函数指针把登记条目与路由配对）。两者分离，供项目自定义
// 自己的包装逻辑，例如不接管渲染的 DocAPI：
//
//	func DocAPI[Req any, Resp any](handler gin.HandlerFunc) gin.HandlerFunc {
//		if xapp.Args.EnableOpenAPI && utils.IsGoRun() {
//			xapp.RegisterAPIDoc[Req, Resp](handler, handler)
//		}
//		return handler
//	}
//
//	// @summary 修改我的资料
//	// @tags im
//	func (i *IMController) UpdateProfile(c *gin.Context) { ... }
//
//	r.PATCH("/im/me/profile", DocAPI[ProfileReq, ProfileResp](ctrl.UpdateProfile))
//
// Req / Resp 仅用于生成文档：GET / DELETE 路由的 Req 生成 query 参数，其余方法
// 生成 application/json requestBody；path 参数由路由 pattern（:id）自动解析，
// 不需要出现在 Req 中。
//
// 本函数不做 EnableOpenAPI 与 go run 判定：那些是 xgo 默认生成流程的策略
// （RegisterAPI 在自身层面判断），自定义生成流程的调用方按需自行把关。
func RegisterAPIDoc[Req any, Resp any](sourceHandler any, wrappedHandler gin.HandlerFunc) bool {
	commentResult, ok := extractHandlerComment(reflect.ValueOf(sourceHandler).Pointer())
	if !ok {
		return false
	}
...
// extractHandlerComment 尽力从 pc 指向的函数源码上方提取 @summary 等文档注释。
// 源码不可得时（如 method value 被归到 <autogenerated>、非 go run 环境读不到
// 源文件）返回 ok=false。
func extractHandlerComment(pc uintptr) (APIComment, bool) {
	handlerFunc := runtime.FuncForPC(pc)
	if handlerFunc == nil {
		xlog.Error("无法获取handler函数信息")
		return APIComment{}, false
	}
	fileName, _ := handlerFunc.FileLine(pc)
	if fileName == "<autogenerated>" {
		// method value（ctrl.Method）的 -fm 包装没有真实源码位置（FileLine 一律
		// 指向 <autogenerated>），但包装符号名保留了完整的包路径 + receiver 类型 +
		// 方法名，改从源码 AST 反查方法定义的 doc 注释。
		return extractMethodValueComment(handlerFunc.Name())
	}
	return extractSourceFileComment(fileName, handlerFunc.Name())
}

// extractSourceFileComment 在 fileName 源码中按符号名定位普通函数定义，提取其
// 上方连续的 // 注释块。与 extractMethodValueComment 对等：一个处理有真实
// FileLine 的普通函数，一个处理只剩符号名的 method value。
func extractSourceFileComment(fileName, funcName string) (APIComment, bool) {
	...
	return parseComments(strings.Join(comments, "\n")), true
}

// extractMethodValueComment 从 method value 包装符号名（pkg/path.(*Recv).Method-fm）
// 反查方法定义并提取 doc 注释。只在 go run 场景下工作：通过向上查找 go.mod 定位
// 模块根，把包路径映射为源码目录，再用 go/parser 找到对应 receiver 的方法声明。
// 任一步失败（方法在依赖包/main 包、源码不可得等）都优雅返回 ok=false。
func extractMethodValueComment(wrapperName string) (APIComment, bool) {
	pkgPath, recvName, methodName, ok := parseMethodWrapperName(wrapperName)
	if !ok {
		return APIComment{}, false
	}
	dir, ok := resolvePackageDir(pkgPath)
	if !ok {
		return APIComment{}, false
	}
	doc, ok := findMethodDoc(dir, recvName, methodName)
	if !ok {
		return APIComment{}, false
	}
	return parseComments(doc), true
}

// parseMethodWrapperName 拆解 -fm 包装符号名。支持指针/值 receiver 与泛型 receiver：
//
//	"pkg/path.(*Ctrl).Method-fm" → ("pkg/path", "Ctrl", "Method")
//	"pkg/path.Ctrl.Method-fm"    → ("pkg/path", "Ctrl", "Method")
//	"pkg/path.(*Ctrl[T]).M-fm"   → ("pkg/path", "Ctrl", "M")
func parseMethodWrapperName(name string) (pkgPath, recvName, methodName string, ok bool) {
	name = strings.TrimSuffix(name, "-fm")
	slash := strings.LastIndex(name, "/")
	tail := name[slash+1:] // "xapp.(*Ctrl).Method"（无 "/" 时为整个 name）
	firstDot := strings.Index(tail, ".")
	if firstDot < 0 {
		return "", "", "", false
	}
	pkgPath = name[:slash+1] + tail[:firstDot]
	rest := tail[firstDot+1:] // "(*Ctrl).Method" 或 "Ctrl.Method"
	lastDot := strings.LastIndex(rest, ".")
	if lastDot < 0 {
		return "", "", "", false
	}
	recvName = strings.TrimSuffix(strings.TrimPrefix(rest[:lastDot], "(*"), ")")
	if idx := strings.Index(recvName, "["); idx >= 0 { // 泛型 receiver 去掉类型参数
		recvName = recvName[:idx]
	}
	methodName = rest[lastDot+1:]
	if pkgPath == "" || recvName == "" || methodName == "" || strings.Contains(recvName, ".") {
		return "", "", "", false
	}
	return pkgPath, recvName, methodName, true
}

// resolvePackageDir 把包导入路径映射为本模块内的源码目录：从 CWD 向上找 go.mod
// 得到模块根与模块路径（go run 通常在项目根执行，与 GenerateOpenAPIDoc 写
// openapi.json 的 CWD 假设一致）。包不属于本模块时返回 false。
func resolvePackageDir(pkgPath string) (string, bool) {
	dir, err := os.Getwd()
	if err != nil {
		return "", false
	}
	for {
		...
		if err == nil {
			...
			if strings.HasPrefix(pkgPath, modPath+"/") {
				return filepath.Join(dir, filepath.FromSlash(strings.TrimPrefix(pkgPath, modPath+"/"))), true
			}
			return "", false
		}
...
// methodDocCache 按目录缓存 findMethodDoc 的解析结果，避免同包多路由重复解析。
// 路由注册与既有 apiRegistry 一样假定发生在启动期单 goroutine。
var methodDocCache = map[string]map[string]string{}

// findMethodDoc 解析 dir 下所有 .go 文件，返回 recvName.methodName 方法声明的
// doc 注释文本（ast.CommentGroup.Text() 格式，已去掉 // 前缀）。
func findMethodDoc(dir, recvName, methodName string) (string, bool) {
	docs, cached := methodDocCache[dir]
	if !cached {
		...
		for _, entry := range entries {
			...
			for _, decl := range f.Decls {
				...
				if !isFunc || fd.Recv == nil || len(fd.Recv.List) == 0 {
					continue
				}
				recv := receiverBaseName(fd.Recv.List[0].Type)
				if recv == "" {
					continue
				}
				var doc string
				...
				docs[recv+"."+fd.Name.Name] = doc
			}
		}
		methodDocCache[dir] = docs
	}
	doc, ok := docs[recvName+"."+methodName]
	return doc, ok
}

// receiverBaseName 取 receiver 类型的基础名：*Ctrl → Ctrl，Ctrl[T] → Ctrl。
func receiverBaseName(expr ast.Expr) string {
	...
	case *ast.Ident:
		return t.Name
	case *ast.IndexExpr: // 泛型 receiver
		return receiverBaseName(t.X)
